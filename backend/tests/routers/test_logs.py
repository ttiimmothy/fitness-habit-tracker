...
class TestLogEndpoints:
  ...
  def test_create_log_multiple_quantity(self, client: TestClient, auth_headers: dict, test_habit: Habit):
    ...
    response = client.post(f"/api/logs/habits/{habit_data['id']}/log",
                           json={
                               "quantity": 3
    },
        headers=auth_headers)
  ...
  def test_create_log_nonexistent_habit(self, client: TestClient, auth_headers: dict):
    ...
    response = client.post("/api/logs/habits/00000000-0000-0000-0000-000000000000/log",
                           json={
                               "quantity": 1
                           },
                           headers=auth_headers)
  ...
  def test_list_logs_success(self, client: TestClient, auth_headers: dict, test_habit: Habit):
    ...
    # Create some logs
    client.post(f"/api/logs/habits/{test_habit.id}/log",
                json={"quantity": 1},
                headers=auth_headers)

    response = client.get(
        f"/api/logs/habits?habit_id={test_habit.id}", headers=auth_headers)
  ...
  def test_list_logs_unauthenticated(self, client: TestClient, test_habit: Habit):
    ...
    response = client.get(f"/api/logs/habits?habit_id={test_habit.id}")
  ...
  def test_list_logs_with_date_filter(self, client: TestClient, auth_headers: dict, test_habit: Habit, db_session: Session):
    ...
    response = client.get(
        f"/api/logs/habits?habit_id={test_habit.id}&date={today.isoformat()}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
    ...
    response = client.get(
        f"/api/logs/habits?habit_id={test_habit.id}&date={yesterday.isoformat()}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
